...
if [ "${1}" = "patches" ]; then
  ...
  getdriveinfo(){
    ...
    if [[ ! $usb ]]; then
        ...
        update_json() {
          ...
          jq -s '.[0] * .[1]' /etc/add_disk_db.json /tmp/new_entry.json > "$tmpfile" && mv "$tmpfile" /etc/add_disk_db.json
        }
    
        if [ -n "${hdmodel}" ] && [ -n "${fwrev}" ]; then
          if [ $(cat "${dbfile}" | grep "${hdmodel}" | wc -l) -gt 0 ]; then
            echo "${hdmodel} is already exists in ${dbfile}, skip writing to /etc/add_disk_db.json" >&2  # debug
          else
            # JSON 파일 업데이트
            if grep '"'"${hdmodel}"'":' /etc/add_disk_db.json >/dev/null; then
  ...
  #cat /etc/add_disk_db.json

  diskdata=$(jq . /etc/add_disk_db.json)
