...
if [ "${1}" = "late" ]; then
  ...
  if [ -f "/tmpRoot/etc/systemd/system/cpuinfo.service" ]; then
    echo "Removing stale /etc/systemd/system/cpuinfo.service override from an earlier loader"
    rm -f "/tmpRoot/etc/systemd/system/cpuinfo.service"
  fi
